...
// }
	...
	async getAllFileData() {
		...
		let fileIdsData = fileIdStrings.map(fileIdString => 
			Promise.resolve(this.getFileData(fileIdString)))

		return Promise.all(fileIdsData)
	}
	...
	async processMatter(content) {
	  ...
	  const contentHtml = preContentHtml.replace(/$basePublicPath$/g, basePublicPath)	  
	  return contentHtml
	}
}